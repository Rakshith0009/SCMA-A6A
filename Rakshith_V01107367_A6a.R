...
# Load the dataset
tisc_data <- read.csv("TISC Historical Data.csv")

# Convert the Date column to Date type using parse_date_time function
tisc_data$Date <- parse_date_time(tisc_data$Date, orders = c("mdy", "dmy", "ymd"))

# Check for missing dates and remove rows with missing dates
tisc_data <- tisc_data %>% drop_na(Date)

# Convert Vol. and Change % from string to numeric
tisc_data$Vol. <- as.numeric(gsub("M", "", tisc_data$Vol.)) * 1e6
tisc_data$Change. <- as.numeric(gsub("%", "", tisc_data$Change.))

# Check for missing values in other columns
sum(is.na(tisc_data))

# Interpolate missing values if there are any
tisc_data <- na_interpolation(tisc_data)

# Check for outliers using boxplot
boxplot(tisc_data$Price, main="Boxplot for Price", ylab="Price")

# Plot the data
ggplot(tisc_data, aes(x = Date, y = Price)) +
  geom_line() +
  labs(title = "Tata steel Stock Price Over Time", x = "Date", y = "Price")

# Split the data into training and testing sets
split_date <- as.Date("2023-12-31")
train_data <- tisc_data %>% filter(Date <= split_date)
test_data <- tisc_data %>% filter(Date > split_date)

# Convert the data to monthly
tisc_data_monthly <- tisc_data %>%
  group_by(month = floor_date(Date, "month")) %>%
  summarise(Price = mean(Price))

# Create time series object
tisc_ts <- ts(tisc_data_monthly$Price, start = c(2020, 1), frequency = 12)

# Decompose the time series using additive model
decomp_additive <- decompose(tisc_ts, type = "additive")

# Decompose the time series using multiplicative model
decomp_multiplicative <- decompose(tisc_ts, type = "multiplicative")

# Plot the decomposed components for additive model
autoplot(decomp_additive) +
  ggtitle("Additive Decomposition of Tata steel Stock Price") +
  theme_minimal()

# Plot the decomposed components for multiplicative model
autoplot(decomp_multiplicative) +
  ggtitle("Multiplicative Decomposition of Tata steel Stock Price") +
  theme_minimal()
...
tisc_ts_daily <- ts(tisc_data$Price, start = c(2020, 1), frequency = 365.25)
tisc_ts_monthly <- ts(tisc_data_monthly$Price, start = c(2020, 1), frequency = 12)

# 1. Holt-Winters model and forecast for the next year
hw_model <- HoltWinters(tisc_ts_monthly)
hw_forecast <- forecast(hw_model, h = 12)
autoplot(hw_forecast) +
  ggtitle("Holt-Winters Forecast for Tata steel Stock Price") +
  theme_minimal()

# 2. Fit ARIMA model to the daily data
arima_model_daily <- auto.arima(tisc_ts_daily)
...
sarima_model_daily <- auto.arima(tisc_ts_daily, seasonal = TRUE)
...
arima_model_monthly <- auto.arima(tisc_ts_monthly)
...
autoplot(monthly_forecast) +
  ggtitle("Monthly ARIMA Forecast for Tata steel Stock Price") +
  theme_minimal()
...
tisc_data <- tisc_data %>% drop_na()

# Split the data into training and testing sets
set.seed(123)
train_index <- createDataPartition(tisc_data$Price, p = 0.8, list = FALSE)
train_data <- tisc_data[train_index, ]
test_data <- tisc_data[-train_index, ]
